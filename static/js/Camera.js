...
var Camera = function (aCanvas, aContext, x, y) {
  ...
  this.update = function (model) {
    ...
    var targetZoom =
      model.camera.maxZoom +
      ((model.camera.minZoom - model.camera.maxZoom) *
        Math.min(model.userTadpole.momentum, model.userTadpole.maxMomentum)) /
    ...
    if (Math.abs(delta.x) + Math.abs(delta.y) > 0.1) {
      ...
      // for(var i = 0, len = model.golds.length; i < len; i++) {
      // 	var gold = model.golds[i];
      // 	gold.x -= (gold.z - 1) * delta.x;
      // 	gold.y -= (gold.z - 1) * o.y;
      // }
    }
  };
