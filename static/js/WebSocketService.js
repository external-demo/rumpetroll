var WebSocketService = function (model, webSocket) {
    ...
    this.welcomeHandler = function (data) {
        ...
        $('#room_num').text('ROOM' + roomNum).show();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    index = i + 1
                    if (index > 3)
                        html += '<tr style="height: 60px;"><td>' + index + '</td><td>' + data[i]["name"] + '</td><td>' + data[i]["golds"] + '</td></tr>'
                    else
                        html += '<tr style="height: 60px;"><td style="background: url(../static/images/num'+ index +'.png) no-repeat center center;"></td><td>' + data[i]["name"] + '</td><td>' + data[i]["golds"] + '</td></tr>'
                }
            },
            error: function () {
